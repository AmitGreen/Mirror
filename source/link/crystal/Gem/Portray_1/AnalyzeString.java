...
//  extends             Object
{
    ...
    private static final boolean        finish()
    {
        ...
        //          '   \   L   "  ra  rq  pc  ps, K  L
        E .overall (U,  K,  L,  T, RA, RQ, PC, PS, x, x);
        K .overall (x,  K,  L,  x, RA, RQ, KC, KS, 7, x);
        L .overall (x,  L,  L,  x,  0,  x, KC, KS, x, 7);
        N .overall (N,  K,  L,  N, RA, RQ, PC, PS, x, x);
        TK.overall (x,  TK, TL, x, RA,  x, KC,  x, 7, x);
        TL.overall (x,  TL, TL, x,  0,  x, KC,  x, x, 7);
        T .overall (x,  TK, TL, x, RA,  x, PC,  x, x, x);
        UK.overall (x,  UK, UL, x, RQ,  x, KS,  x, 7, x);
        UL.overall (x,  UL, UL, x,  0,  x, KS,  x, x, 7);
        U .overall (x,  UK, UL, x, RQ,  x, PS,  x, x, x);
...
//      trace("{+}: Going to call: {}", normal.abbreviation);
    ...
    public static final void            dump()
    {
        ...
        trace("   E:  {p}", AnalyzeString.E);
        trace("   K:  {p}", AnalyzeString.K);
        trace("   L:  {p}", AnalyzeString.L);
        trace("   N:  {p}", AnalyzeString.N);
        trace("  TK:  {p}", AnalyzeString.TK);
        trace("  TL:  {p}", AnalyzeString.TL);
        trace("   T:  {p}", AnalyzeString.T);
        trace("  UK:  {p}", AnalyzeString.UK);
        trace("  UL:  {p}", AnalyzeString.UL);
        trace("   U:  {p}", AnalyzeString.U);
